...
import { AgentCard, IndicativePriceNote } from "@/components/commerce/inquiry-ui";
    ...
    <div className="space-y-4">
      <Card className="p-5">
        ...
        <p className="mt-1 text-sm text-ink-600">
          A real person on the OUTLY team — India and UAE shifts, so someone is on between 9 am and
          11 pm IST.
        </p>
        <div className="mt-3 space-y-2">
          {agents.slice(0, compact ? 2 : 3).map((a) => (
            <AgentCard key={a.id} agent={a} />
          ))}
        </div>
      </Card>
      <Card className="p-5">
        <h2 className="mb-3 text-[0.95rem]">Why ask OUTLY</h2>
        <TrustSummary />
      </Card>
    </div>
  );
